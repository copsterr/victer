...
from lane_image_functions import *


""" Window Searching """
def laneWindowSearch(bin_image, debug=False):
    nwindows = 9 # number of windows in vertical axis
...
def getLaneOffset(image, left_fit, right_fit, imsize=(720, 1280), x_mpp=3.7 / 900):
    warp_ratio = 2.56
    ...
    # print(f"left start: {left_lane_start},  right start: {right_lane_start}")
    ...
    cv.putText(image, "Car Center", (int(img_center) -150, img_h - 50), cv.FONT_HERSHEY_SIMPLEX, 0.8, BGR_RED, 2)
    
    # Draw Line Center
    if dist != 0:
        cv.line(image, (int(lane_center), img_h - 20), (int(lane_center), int(img_h) - 80), BGR_BLUE, 2) # draw lane center
        cv.putText(image, "Lane Center", (int(lane_center) + 10, img_h - 30), cv.FONT_HERSHEY_SIMPLEX, 0.8, BGR_BLUE, 2)
